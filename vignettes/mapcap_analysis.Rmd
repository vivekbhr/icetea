...
In order to evaluate whether spike-in reads are produced in consistant proportion to total reads, we provide a convinient function `plotSpikeProportion`.

```{r spikeInEval, eval=FALSE}

```


Normalization factors can be obtained from the spike-in reads using `getNormFactors` and provided to the `firDiffTSS`
